...
bounds <- list(
  years_of_educ = c(min(asec_female$years_of_educ), max(asec_female$years_of_educ)), 
  potential_experience = c(min(asec_female$potential_experience), max(asec_female$potential_experience)),
  potential_experience_squared = c(min(asec_female$potential_experience_squared), max(asec_female$potential_experience_squared)),
  potential_experience_cubed = c(min(asec_female$potential_experience_cubed), max(asec_female$potential_experience_cubed)),
  non_white = c("0", "1"),
  log_INCWAGE = c(min(asec_female$log_INCWAGE), max(asec_female$log_INCWAGE))
)
...
# lm_wishart(
#   formula = model_formula,
#   data = as.data.frame(asec_female),
#   epsilon = 1,
#   delta = 1,
#   bounds = bounds,
#   var_names = var_names,
...
lm_wklm_pure_test <- future_map(
  ...
  .options = furrr::furrr_options(seed = 271944),
  .progress = TRUE
)
toc()
...
write_csv(coefficient_results_asymptotic, file = here::here("results", "04_cps_female-regression_coefficient_asymptotic.csv"))
...
write_csv(coefficient_results_bootstrap, file = here::here("results", "04_cps_female-regression_coefficient_bootstrap.csv"))
...
write_csv(rmse_results_asymptotic, file = here::here("results", "04_cps_female-regression_rmse_asymptotic.csv"))
...
write_csv(rmse_results_bootstrap, file = here::here("results", "04_cps_female-regression_rmse_bootstrap.csv"))
