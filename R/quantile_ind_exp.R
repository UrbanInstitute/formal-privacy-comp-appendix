#' Calculate formally private quantiles with ind_exp
#'
...
#' 
quantile_ind_exp <- function(x, probs, epsilon, delta, data_low, data_high) {
  ...
  ind_exp <- reticulate::source_python(file = here::here("dp-code", "quantile-functions", "ind_exp.py"))
  gcpy <- reticulate::import(module = "gc")

  x <- reticulate::r_to_py(x) # needed on older versions of reticulate
  
  ind_opt <- py$opt_comp_calculator(
    overall_eps = epsilon, 
    overall_delta = delta, 
    num_comps = length(probs)
  )
  
  quantiles <- py$ind_exp(
    sorted_data = x, 
    data_low = data_low, 
    data_high = data_high, 
    qs = probs, 
